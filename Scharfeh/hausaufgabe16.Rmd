...
Was beeinflusst den Preis mehr: die Farbe oder der Schliff eines Diamanten?

Einfluss des Schliffs (cut) auf den Preis:

```{r}
ggplot(diamonds, aes(x=cut, y=price))+ facet_wrap (~clarity) +geom_point()
```

Einfluss der Farbe (im Datensatz fälschlich color) auf den Preis des Diamanten:
```{r}
ggplot(diamonds, aes(x=color, y=price, colour=color))+facet_wrap (~cut)+ geom_point()
